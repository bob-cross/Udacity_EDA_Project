...
```
Next, I wanted look at the maximum, minimum and quartile values for each variable. I also wanted to couple that summary with a boxplot of the variables representation in each quality category. This section is presented in the order as the names are listed above. 
...
```
```{r echo=FALSE}
ggplot(redwine,aes(x=quality.f, y=fixed.acidity))+ geom_boxplot()
```
...
```
```{r echo=FALSE}
ggplot(redwine,aes(x=quality.f, y=volatile.acidity))+ geom_boxplot()
```
...
```
```{r echo=FALSE}
ggplot(redwine,aes(x=quality.f, y=citric.acid))+ geom_boxplot()
...
```
```{r}
ggplot(redwine,aes(x=quality.f, y=residual.sugar))+ geom_boxplot()
```
...
```
```{r}
ggplot(redwine,aes(x=quality.f, y=chlorides))+ geom_boxplot()
```
...
```
```{r}
ggplot(redwine,aes(x=quality.f, y=total.sulfur.dioxide))+ geom_boxplot()
```
...
```
```{r echo=FALSE}
ggplot(redwine,aes(x=quality.f, y=density))+
  geom_boxplot()
```
...
```
```{r echo=FALSE}
ggplot(redwine,aes(x=quality.f, y=pH))+ geom_boxplot()
```
...
```
```{r echo=FALSE}
fp2b <-ggplot(redwine,aes(x=quality.f, y=sulphates))+ geom_boxplot()
...
```{r echo=FALSE}
fp2a <- ggplot(redwine,aes(x=quality.f, y=alcohol))+ geom_boxplot()
...
```
Now, with some understanding of the dataset Quality distribution and each variables distribution over quality, I wanted to dig a bit deeper on each variable. I noted from the variable summary information that some of the variables appeared to have a normal distribution while others did not. Thus, digging further, I wanted to see if transforming some of the variables would show a more revealing distribution and potential better use for regression analysis.
```{r echo=FALSE}
...
free_to_total_ratio_p1 <- ggplot(aes(x = free_to_total_ratio), data = redwine) +
...
Building on insight from the previous Univariate review, I decided to group several of the variables at a time for analysis. The analysis below shows graphs and Pearson Correlation values using the ggpairs function.
...
The next several plots look at the four variables which show the strongest in fluenece on quality: alcohol, volatile acidity, citric acid and sulphates.  
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
fp3 <- ggplot(aes(x = alcohol, y = sulphates), data = redwine) +
  geom_point(aes(color = quality.f)) +
  ylab('sulphaes') +
  xlab('alcohol, % by volume') +
  scale_y_continuous(limits = c(0, 0.8)) +
...
ggplot(aes(x = alcohol, y = citric.acid/volatile.acidity), data = redwine) +
  ...
  theme(text = element_text(size = 8))
```
The next several plots look at the strongest correletions between the other input variables: the group of density, fixed acidity, residual sugar seemed of interest as did the group of fixed acidity, free sulfur dioxide, total sulfur dioxide.
Before diving into those groups, I wanted to look at quality in relation to density and residual sugar. As a person who enjoys red wine, I have a personal preference for what I describe as a'red whine which is kinda heavy with a touch of sweetness'.
```{r echo=FALSE}
ggplot(aes(x = quality, y = residual.sugar/density), data = redwine) +
  ...
  scale_x_log10() +
  scale_color_brewer(type = 'seq', palette = 'YlOrBr') +
  ggtitle('Quality vs Residual Sugar / Density in Red Wines') +
  theme(text = element_text(size = 8))
```
...
ggplot(aes(x = fixed.acidity, y = free.sulfur.dioxide / total.sulfur.dioxide), data = redwine) +
  geom_point(aes(color = quality.f)) +
  xlab('fixed acidity') +
  ylab('free sulfur dioxide / total sulfur dioxide') +
  scale_y_log10() +
  scale_x_log10() +
  scale_color_brewer(type = 'seq', palette = 'YlOrBr') +
  ggtitle('Fixed Acidity vs Free Sulfur Dioxide / Total Sulfur Dioxide') +
  theme(text = element_text(size = 8))
```
...
Most of the Red Wines have a Quality rating between 5 and 7.
```{r echo=FALSE, warning=FALSE, message=FALSE}
fp1
```
Description Plot Two

Quality ratings for Red Wines increase with increased levels of Alcohol and Sulphates.
```{r echo=FALSE}
grid.arrange(fp2a,fp2b,ncol=2)
```
Description Plot Three

The final plot with sulphates vs alcohol relative to quality indicates that sulphate content has a greater bearing on quality.
```{r echo=FALSE,message=FALSE,warning=FALSE}
fp3
```
Reflections
